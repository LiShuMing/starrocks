...
public:
    ...
    virtual Ptr replicate(const Buffer<uint32_t>& offsets) {
    ...
    using ColumnCallback = std::function<void(Column*)>;
    virtual void for_each_subcolumn(ColumnCallback) {}

    MutablePtr mutate() const&& {
        MutablePtr res = shallow_mutate();
        res->for_each_subcolumn([](Column* subcolumn) { subcolumn = std::move(*subcolumn).mutate(); });
        return res;
    }
...
protected:
    static StatusOr<Ptr> downgrade_helper_func(WrappedPtr* col);
    static StatusOr<Ptr> upgrade_helper_func(WrappedPtr* col);
...
using WrappedPtr = Column::WrappedPtr;
using WrappedColumns = std::vector<WrappedPtr>;
