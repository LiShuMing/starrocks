...
public:
    ...
    using DataColumnMutablePtr = DataColumn::MutablePtr;
    using NullColumnMutablePtr = NullColumn::MutablePtr;
    using DataColumnWrappedPtr = DataColumn::DerivedWrappedPtr;
    using NullColumnWrappedPtr = NullColumn::DerivedWrappedPtr;
    ...
    MutableColumnPtr build(bool is_const) {
        ...
        if (is_const) {
            return ConstColumn::create(std::move(_column)->assume_mutable(), _column->size());
        } else if (_has_null) {
            return NullableColumn::create(std::move(_column)->assume_mutable(),
                                          std::move(_null_column)->assume_mutable());
        } else {
            return std::move(_column)->assume_mutable();
        }
    }

    MutableColumnPtr build_nullable_column() {
        return NullableColumn::create(std::move(_column)->assume_mutable(), std::move(_null_column)->assume_mutable());
    }
...
protected:
    DataColumnWrappedPtr _column;
    NullColumnWrappedPtr _null_column;
    bool _has_null;
};
