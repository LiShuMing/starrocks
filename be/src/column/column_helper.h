...
public:
    ...
    static ColumnPtr unpack_and_duplicate_const_column(size_t chunk_size, const ColumnPtr& column) {
        if (column->is_constant()) {
            // TODO(COW): we should not modify the const column's data directly
            auto* const_column = down_cast<ConstColumn*>(column->assume_mutable().get());
    ...
    static MutableColumnPtr unfold_const_column(const TypeDescriptor& type_desc, size_t size,
        ...
        if (column->only_null()) {
            ...
            // TODO: Whether to change original column or not.
            auto* const_column = down_cast<ConstColumn*>(column.get());
            const_column->mutable_data_column()->assign(size, 0);
            return const_column->data_column()->assume_mutable();
        } else {
            return std::move(column);
        }
    }
        ...
        if (src_column->is_nullable()) {
            ...
            if (!dst_nullable) {
                ...
                ColumnPtr nullable_column =
                        NullableColumn::create(src_column->assume_mutable(), NullColumn::create(num_rows, 0));
    ...
    static MutableColumnPtr cast_to_nullable_column(ColumnPtr src_column) {
        if (src_column->is_nullable()) {
            return src_column->assume_mutable();
        }
        return NullableColumn::create(src_column->assume_mutable(), NullColumn::create(src_column->size(), 0));
    }
    ...
    //         return down_cast<const RunTimeColumnType<Type>*>(value.get());
    //     }
