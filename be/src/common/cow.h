...
protected:
    ...
    public:
        ...
        // Implicit conversion operator to base type (for reference parameters)
        // This allows: MutPtr<Derived> to be implicitly converted to MutPtr<Base>
        template <typename BaseType, typename = std::enable_if_t<std::is_base_of_v<BaseType, T>>>
        operator MutPtr<BaseType>() & {
            // Create a new MutPtr pointing to the same object, adding a reference
            if (this->get() != nullptr) {
                this->get()->add_ref();
            }
            return MutPtr<BaseType>(this->get(), false); // false because we already added ref
        }
        MutPtr() = default;
...
public:
    ...
    MutablePtr as_mutable_ptr() const {
        DCHECK_LE(use_count(), 2) << "as_mutable_ptr() called on heavily shared object (use_count=" << use_count()
    ...
    Derived* as_mutable_raw_ptr() const {
        ...
        DCHECK_LE(use_count(), 2) << "as_mutable_raw_ptr() called on heavily shared object (use_count=" << use_count()
