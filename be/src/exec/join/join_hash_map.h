...
private:
    ...
    void _copy_probe_column(MutableColumnPtr* src_column, ChunkPtr* chunk, const SlotDescriptor* slot,
                            bool to_nullable) {
        if (_probe_state->match_flag == JoinMatchFlag::ALL_MATCH_ONE) {
            if (to_nullable) {
                MutableColumnPtr dest_column =
                        NullableColumn::create(std::move(*src_column), NullColumn::create(_probe_state->count));
    ...
    void _copy_probe_nullable_column(MutableColumnPtr* src_column, ChunkPtr* chunk, const SlotDescriptor* slot) {
        if (_probe_state->match_flag == JoinMatchFlag::ALL_MATCH_ONE) {
            (*chunk)->append_column(std::move(*src_column), slot->id());
        } else {
