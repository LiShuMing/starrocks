...
Status LocalExchangeSourceOperator::set_epoch_finished(RuntimeState* state) {
...
ChunkPtr LocalExchangeSourceOperator::_pull_shuffle_chunk(RuntimeState* state) {
    ...
    auto* chunk_extra_data =
            first_partition_chunk->get_extra_data()
                    ? dynamic_cast<ChunkExtraColumnsData*>(first_partition_chunk->get_extra_data().get())
                    : nullptr;
    if (chunk_extra_data) {
