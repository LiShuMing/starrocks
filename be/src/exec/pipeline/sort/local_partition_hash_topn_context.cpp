...
LocalPartitionHashTopnContext::LocalPartitionHashTopnContext(
        const std::vector<TExpr>& t_partition_exprs, bool has_nullable_key, const std::vector<ExprContext*>& sort_exprs,
        std::vector<bool> is_asc_order, std::vector<bool> is_null_first, std::string sort_keys, int64_t offset,
        int64_t partition_limit, const std::vector<TExpr>& t_pre_agg_exprs,
        const std::vector<TSlotId>& t_pre_agg_output_slot_id, bool enable_pre_agg)
    ...
    DCHECK(enable_pre_agg) << "enable_pre_agg must be true";
}
...
Status LocalPartitionHashTopnContext::prepare(RuntimeState* state, RuntimeProfile* runtime_profile) {
    ...
    _pre_agg = std::make_unique<LocalPartitionPreAggregate>(_t_pre_agg_exprs, _t_pre_agg_output_slot_id,
                                                            _mem_pool.get(), false);
    RETURN_IF_ERROR(_pre_agg->prepare(state));
    _chunks_partitioner =
            std::make_shared<ChunksPartitioner>(_has_nullable_key, _partition_exprs, _partition_types, _mem_pool.get());
    RETURN_IF_ERROR(_chunks_partitioner->prepare(state, runtime_profile, true));
...
Status LocalPartitionHashTopnContext::push_one_chunk(RuntimeState* state, const ChunkPtr& chunk) {
    auto new_partition_cb = [this](size_t partition_idx) {
        (void)_hash_topn->new_partition(partition_idx);
        AggDataPtr agg_states =
                _mem_pool->allocate_aligned(_pre_agg->_agg_states_total_size, _pre_agg->_max_agg_state_align_size);
        _pre_agg->_managed_fn_states.emplace_back(
                std::make_unique<LocalPartitionPreAggregateState>(&_pre_agg->_agg_fn_ctxs, agg_states, _pre_agg.get()));
    };
    auto partition_consumer = [this](size_t partition_idx, const ChunkPtr& partition_chunk) {
        ...
        (void)_pre_agg->compute_agg_state(partition_chunk.get(), partition_idx);
    };
    auto partition_validator = [this](size_t partition_idx, const Columns& partition_columns, size_t row_idx) {
        return _hash_topn->is_valid(partition_idx, partition_columns, row_idx);
    };
    ASSIGN_OR_RETURN(auto partition_columns, _compute_partition_columns(chunk));
    return _chunks_partitioner->offer<false>(chunk, partition_columns, std::move(new_partition_cb),
                                             std::move(partition_consumer), std::move(partition_validator));
}

StatusOr<Columns> LocalPartitionHashTopnContext::_compute_partition_columns(const ChunkPtr& chunk) {
...
void LocalPartitionHashTopnContext::sink_complete() {
    VLOG_ROW << "LocalPartitionHashTopnContext sink complete";
...
Status LocalPartitionHashTopnContext::finalize(RuntimeState* state) {
    ...
    if (!_chunks_partitioner->is_hash_map_eos()) {
        RETURN_IF_ERROR(
                _chunks_partitioner->consume_from_hash_map([this, state](int32_t partition_idx, const ChunkPtr& chunk) {
                    (void)_hash_topn->offer(static_cast<size_t>(partition_idx), chunk);
                    (void)_pre_agg->compute_agg_state(chunk.get(), partition_idx);
                    return true;
                }));
    }
    ...
    _is_finalized = true;
    _mem_pool.reset();
    VLOG_ROW << "LocalPartitionHashTopnContext finalize, partition num: " << _partition_num;
    return Status::OK();
}

bool LocalPartitionHashTopnContext::has_output() const {
    if (_is_finalized && _chunks_partitioner->is_passthrough()) {
        return !_chunks_partitioner->is_passthrough_buffer_empty();
    }
    return _is_sink_complete && _hash_topn->has_output();
}
...
StatusOr<ChunkPtr> LocalPartitionHashTopnContext::pull_one_chunk() {
    ...
    ASSIGN_OR_RETURN(chunk, _hash_topn->get_group_by_sort_columns(chunk_size, partition_idxes));
    if (chunk != nullptr) {
        _pre_agg->output_agg_result(chunk.get(), partition_idxes, false);
        return chunk;
    }
    return nullptr;
}
...
LocalPartitionHashTopnContext* LocalPartitionHashTopnContextFactory::create(int32_t driver_sequence) {
    ...
    auto ctx = std::make_shared<LocalPartitionHashTopnContext>(
            _t_partition_exprs, false, _sort_exprs, _is_asc_order, _is_null_first, _sort_keys, _offset, _limit,
            _t_pre_agg_exprs, _t_pre_agg_output_slot_id, _enable_pre_agg);
    auto* raw = ctx.get();
