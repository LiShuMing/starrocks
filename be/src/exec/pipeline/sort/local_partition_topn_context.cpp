    ...
    RETURN_IF_ERROR(_chunks_partitioner->offer<true>(
            ...
            [this, state](size_t partition_idx) {
                ...
                if constexpr (EnablePreAgg) {
                    ...
                    _pre_agg->_managed_fn_states.emplace_back(std::make_unique<ManagedPreAggregateStates>(
...
StatusOr<Columns> LocalPartitionTopnContext::_compute_partition_columns(const ChunkPtr& chunk) {
...
StatusOr<ChunkPtr> LocalPartitionTopnContext::_pull_one_chunk_impl() {
    ...
    if constexpr (EnablePreAgg) {
        if (chunk != nullptr) {
            RETURN_IF_ERROR(_pre_agg->output_agg_streaming(chunk.get()));
        }
    }
    return chunk;
}
...
StatusOr<ChunkPtr> LocalPartitionTopnContext::_pull_one_chunk_from_sorters_impl() {
    ...
    if constexpr (EnablePreAgg) {
        _pre_agg->output_agg_result(chunk.get(), _sorter_index, eos, _pre_agg->_is_first_chunk_of_current_sorter);
...
LocalPartitionTopnContext* LocalPartitionTopnContextFactory::create(int32_t driver_sequence) {
    ...
    auto ctx = std::make_shared<LocalPartitionTopnContext>(
            _t_partition_exprs, _has_outer_join_child, _enable_pre_agg, _t_pre_agg_exprs, _t_pre_agg_output_slot_id,
