...
StatusOr<DriverState> StreamPipelineDriver::process(RuntimeState* runtime_state, int worker_id) {
    ...
    while (true) {
        ...
        for (size_t i = _first_unfinished; i < num_operators - 1; ++i) {
            {
                ...
                if (return_status.ok()) {
                    if (maybe_chunk.value()) {
                        if (maybe_chunk.value()->num_rows() > 0) {
                        ...
                        } else if (vectorized::BarrierChunkConverter::is_barrier_chunk(maybe_chunk.value())) {
                            // pass through Barrier Chunk
                            {
                                SCOPED_TIMER(next_op->_push_timer);
                                QUERY_TRACE_SCOPED(next_op->get_name(), "push_chunk");
                                return_status = next_op->push_chunk(runtime_state, maybe_chunk.value());
                                _first_epoch_unfinished = i + 1;
                            }
                        } else {
                            // do nothing if maybe_chunk's num_row is 0.
                        }
                    }
                    num_chunks_moved += 1;
                    total_chunks_moved += 1;
                } else if (!return_status.ok() && !return_status.is_end_of_file()) {
        ...
        if (_first_epoch_unfinished == num_operators) {
            should_yield = true;
        }
