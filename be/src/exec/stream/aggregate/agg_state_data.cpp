...
Status AggStateData::allocate_retract_state(size_t chunk_size, const std::vector<uint8_t>& keys_not_in_map,
    ...
    for (auto i = 0; i < chunk_size; i++) {
        ...
        if (keys_not_in_map[i]) {
            DCHECK_LT(j, chunk_size);
            // replacement new
            VLOG_ROW << "allocate_retract_state, key not in map:" << i << ", agg_func_idx:" << agg_func_idx;
//            _agg_function->create(_agg_fn_ctx, agg_group_state[i] + _agg_state_offset);
            auto& result_chunk_or = result_chunks[j++];
            if (result_chunk_or.ok()) {
                ...
                do {
                    ...
                    for (auto& col: columns) {
                        std::cout << "restore detail, col:" << col->debug_string() << std::endl;
                    }
                    _agg_function->restore_detail(_agg_fn_ctx,
                                                  t_chunk->num_rows(),
                                                  columns,
...
Status AggStateData::process_chunk(size_t chunk_size,
                                   const std::vector<ChunkPtrOr>& result_chunks,
    ...
    RETURN_IF_ERROR(allocate_intermediate_state(chunk_size, keys_not_in_map, result_chunks, agg_group_state));
...
Status AggStateData::output_result(size_t chunk_size,
                                   const Buffer<vectorized::AggDataPtr>& agg_group_data,
                                   vectorized::Column* to)  const {
    if (_agg_state_kind == StreamStateTableKind::Detail_Intermediate ||
        _agg_state_kind == StreamStateTableKind::Detail_Result) {
...
Status AggStateData::output_detail(size_t chunk_size,
                                   const Buffer<vectorized::AggDataPtr>& agg_group_data,
                                   const vectorized::Columns& to,
                                   vectorized::Column* count) const {
    for (size_t i = 0; i < chunk_size; i++) {
...
Status IntermediateAggGroupState::process_chunk(size_t chunk_size,
    ...
    for (auto& agg_state: _agg_states) {
        // Allocate state by using intermediate states.
        RETURN_IF_ERROR(agg_state->process_chunk(chunk_size, result_chunks, keys_not_in_map, ops,
...
Status DetailAggGroupState::process_chunk(size_t chunk_size,
                                          const Buffer<DatumRow>& non_found_keys,
                                          const Buffer<uint8_t>& keys_not_in_map,
                                          const StreamRowOp* ops,
                                          std::vector<std::vector<const vectorized::Column*>>& raw_columns,
    ...
    for (auto& agg_state : _agg_states) {
        ...
        if (agg_state->state_table_kind() == StreamStateTableKind::Detail_Result) {
            ...
            DCHECK(agg_state->state_table_kind() == StreamStateTableKind::Detail_Intermediate);
