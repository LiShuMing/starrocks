...
using AggStateTableKind = vectorized::AggStateTableKind;

struct AggStateDataParams {
    int32_t func_idx;
    int32_t intermediate_table_idx; // maybe result/intermediate table decided by AggStateTableKind
...
public:
    AggStateData(const vectorized::AggregateFunction* func, starrocks_udf::FunctionContext* agg_fn_ctx,
        ...
        VLOG_ROW << "agg_state_kind=" << agg_state_kind << " func_idx=" << _params.func_idx
                 ...
                 << " retract_table_idx=" << _params.retract_table_idx;
    }
    ...
    Status allocate_retract_state(size_t chunk_size, const std::vector<uint8_t>& keys_not_in_map,
                                  const vectorized::Column* raw_column, const std::vector<ChunkPtrOr>& result_chunks,
                                  const Buffer<AggGroupStatePtr>& agg_group_state) const;

    Status process_chunk(size_t chunk_size, const std::vector<ChunkPtrOr>& result_chunks,
    ...
    Status finalize_result(size_t chunk_size, const Buffer<vectorized::AggDataPtr>& agg_group_data,
                           vectorized::Column* to) const;
    Status finalize_detail_result(size_t chunk_size, const vectorized::Columns& group_by_columns,
    ...
    const bool is_detail_state_table() const {
...
private:
    std::vector<DatumRow> _construct_non_found_detail_keys(size_t chunk_size, const Buffer<DatumRow>& non_found_keys,
                                                           std::vector<const vectorized::Column*> agg_cols) const {
        ...
        for (int i = 0; i < agg_column_size; i++) {
            ...
            for (int j = 0; j < chunk_size; j++) {
                seek_keys[j].push_back(agg_col->get(j));
            }
        }
        return seek_keys;
    }
