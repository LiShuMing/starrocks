...
Status StreamAggregator::_prepare_state_tables(RuntimeState* state) {
    ...
    for (int32_t i = 0; i < agg_size; i++) {
        auto agg_state_kind = _agg_functions[i]->stream_state_table_kind(_params->is_generate_retract);
        switch (agg_state_kind) {
        case StreamStateTableKind::Result: {
            auto agg_state = std::make_unique<AggStateData>(_agg_functions[i],
                                                            _agg_fn_ctxs[i],
                                                            agg_state_kind,
        ...
        case StreamStateTableKind::Intermediate: {
            auto agg_state = std::make_unique<AggStateData>(_agg_functions[i],
                                                            _agg_fn_ctxs[i],
                                                            agg_state_kind,
                                                            _agg_states_offsets[i],
        ...
        case StreamStateTableKind::Detail_Result: {
            // For detail agg funcs, just use intermediate agg_state when no need generate retracts.
            DCHECK(_params->is_generate_retract);
            auto agg_state = std::make_unique<AggStateData>(_agg_functions[i],
                                                            _agg_fn_ctxs[i],
                                                            StreamStateTableKind::Detail_Result,
                                                            _agg_states_offsets[i],
        ...
        case StreamStateTableKind::Detail_Intermediate: {
            auto agg_state = std::make_unique<AggStateData>(_agg_functions[i],
                                                            _agg_fn_ctxs[i],
                                                            StreamStateTableKind::Detail_Intermediate,
                                                            _agg_states_offsets[i],
    ...
    if (_params->is_testing) {
        ...
        auto output_slots_without_op = std::vector<SlotDescriptor*>({output_slots.begin(), output_slots.end() - 1});
        _result_state_table = std::make_unique<MemStateTable>(output_slots_without_op, key_size, true);

        // intermediate agg_state is created when intermediate/detail agg states are not empty.
        if (!_intermediate_agg_func_ids.empty()) {
            _intermediate_state_table = std::make_unique<MemStateTable>(_intermediate_tuple_desc->slots(),
                                                                        key_size, false);
        }
        ...
        if (!detail_agg_states.empty()) {
            ...
            for (auto& agg_state: detail_agg_states) {
                ...
                auto detail_state_table = std::make_unique<MemStateTable>(detail_table_slots,
                                                                          key_size + 1, false);
    ...
    if (!result_agg_states.empty()) {
        _result_agg_group = std::make_unique<IntermediateAggGroupState>(std::move(result_agg_states),
                                                                        _result_state_table.get());
    }
    ...
    if (!detail_agg_states.empty()) {
        ...
        _detail_agg_group = std::make_unique<DetailAggGroupState>(std::move(detail_agg_states),
                                                                  _result_state_table.get(),
                                                                  _intermediate_state_table.get(),
                                                                  _detail_state_tables);
    }
    return Status::OK();
}

Status StreamAggregator::process_chunk(vectorized::Chunk* chunk) {
    ...
    // TODO(lism): Maybe we can compact build_hash_map and compute_not_founds into one method.
    {
        SCOPED_TIMER(agg_compute_timer());
        TRY_CATCH_BAD_ALLOC(hash_map_variant().visit([&](auto& hash_map_with_key) {
            build_hash_map(*hash_map_with_key, chunk_size);
        }));
    }
    ...
    if (_result_agg_group) {
        _result_agg_group->process_chunk(chunk_size, _non_found_keys,
                                         _streaming_selection,
                                         ops, _agg_input_raw_columns, _tmp_agg_states);
    }
    if (_intermediate_agg_group) {
        _intermediate_agg_group->process_chunk(chunk_size, _non_found_keys,
                                               _streaming_selection,
                                               ops, _agg_input_raw_columns, _tmp_agg_states);
    }
    if (_detail_agg_group) {
        _detail_agg_group->process_chunk(chunk_size, _non_found_keys,
                                         _streaming_selection,
                                         ops, _agg_input_raw_columns, _tmp_agg_states);
    }
...
Status StreamAggregator::output_changes(int32_t chunk_size, vectorized::ChunkPtr* result_chunk,
...
Status StreamAggregator::_output_changes(HashMapWithKey& hash_map_with_key,
                                         int32_t chunk_size, vectorized::ChunkPtr* result_chunk,
                                         vectorized::ChunkPtr* intermediate_chunk,
                                         std::vector<vectorized::ChunkPtr>& detail_chunks) {
...
Status StreamAggregator::_output_intermediate_changes(int32_t chunk_size,
    ...
    for (auto& i: _intermediate_agg_func_ids) {
        _agg_functions[i]->batch_serialize(_agg_fn_ctxs[i], chunk_size, _tmp_agg_states,
                                           _agg_states_offsets[i], agg_intermediate_columns[i].get());
    }
...
Status StreamAggregator::_output_result_changes(int32_t chunk_size,
                                                const vectorized::Columns& group_by_columns,
                                                ChunkPtr* result_chunk) {
    if (_params->is_generate_retract) {
        ChunkPtr prev_result_chunk;
        vectorized::Buffer<uint8_t> prev_existence;
        RETURN_IF_ERROR(_output_result_prev_retract_changes(chunk_size, group_by_columns,
                                                            &prev_result_chunk, &prev_existence));

        RETURN_IF_ERROR(_output_result_post_retract_changes(chunk_size, group_by_columns, prev_existence,
                                                            prev_result_chunk, result_chunk));
    } else {
...
Status StreamAggregator::_output_result_post_retract_changes(size_t chunk_size,
    ...
    auto agg_count_column = reinterpret_cast<vectorized::Int64Column*>(post_agg_result_columns[_count_agg_idx].get());
    auto& count_agg_state = _agg_func_states[_count_agg_idx];
    RETURN_IF_ERROR(count_agg_state->output_result(chunk_size, _tmp_agg_states, agg_count_column));
    std::cout << "agg_count_column:" << agg_count_column->debug_string() << std::endl;
    ...
    auto* prev_ops_data = prev_result_chunk->ops();
    for (size_t i = 0; i < chunk_size; i++) {
        if (agg_count_column_data[i] == 0) {
            _streaming_selection[i] = 1;

            if (prev_existence[i]) {
                // if key's count is down to zero, change its op to DELETE_OP.
                prev_ops_data[i] = DELETE_OP;
            }
        }
    }
    ...
    if (selected_count > 0) {
        ...
        for (auto& key_col: post_group_by_columns) {
            ...
            std::cout << " output post group by :"  << ", debug_string:" << key_col->debug_string()  << std::endl;
        }

        // filter agg columns
        for (size_t i = 0; i < _agg_fn_ctxs.size(); i++) {
            post_agg_result_columns[i]->filter(filter, chunk_size);
            std::cout << " output post aggregate:  i: " << i << ", debug_string:" << post_agg_result_columns[i]->debug_string()  << std::endl;
        }
        ...
        std::cout << " output post ops:"  << ", debug_string:" << post_ops->debug_string()  << std::endl;

        auto post_chunk_result = _build_output_chunk_with_ops(post_group_by_columns, post_agg_result_columns, post_ops, false);
...
Status StreamAggregator::_output_result_prev_retract_changes(size_t chunk_size,
    ...
    for (size_t i = 0; i < chunk_size; i++) {
        ...
        std::cout << "prev result column:" << prev_result->num_columns()
                  << ", agg_result_column size: " << prev_agg_result_columns.size() << std::endl;
    ...
    if (prev_count == 0) {
        *prev_result_chunk = _build_output_chunk_with_ops(prev_group_by_columns, prev_agg_result_columns, prev_ops, false);
    } else {
        // append group by columns
        for (size_t idx = 0; idx < prev_group_by_columns.size(); idx++) {
            // TODO: add append_by_filter method.
            prev_group_by_columns[idx]->append_selective(*(group_by_columns[idx]),
                                                         permutation.data(), 0, permutation.size());
            std::cout << " output prev group by column:"  << ", debug_string:" << prev_group_by_columns[idx]->debug_string()  << std::endl;
        }
        for (auto& agg_col: prev_agg_result_columns) {
            std::cout << " output prev agg column:"  << ", debug_string:" << agg_col->debug_string()  << std::endl;
        }
        // op col
        prev_ops->append_value_multiple_times(&UPDATE_BEFORE_OP, prev_count);
        std::cout << " output prev ops:"  << ", debug_string:" << prev_ops->debug_string()  << std::endl;
...
Status StreamAggregator::_output_retract_detail(int32_t chunk_size, const vectorized::Columns& group_by_columns,
    ...
    for (size_t i = 0; i < _detail_agg_group->agg_states().size(); i++) {
        ...
        auto agg_func_idx = agg_state->agg_func_id();
        std::cout << "[_output_retract_detail] i : " << i << ", func_id:" << agg_func_idx << std::endl;
        ...
        auto result_count = vectorized::ColumnHelper::create_column(
                _agg_fn_types[_count_agg_idx].result_type, false);
        ...
        for (size_t j = 0; j < group_by_columns.size(); j++) {
            ...
            std::cout << "[output_retract_detail] group by col:" << replicated_col->debug_string() << std::endl;
            detail_result_chunk->append_column(replicated_col, slot_id++);
        }
        std::cout << "[output_retract_detail] agg col:" << agg_col->debug_string() << std::endl;
        std::cout << "[output_retract_detail] count col:" << count_col->debug_string() << std::endl;
