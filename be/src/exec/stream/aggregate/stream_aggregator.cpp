...
Status StreamAggregator::process_chunk(vectorized::Chunk* chunk) {
...

    {
        ...
        TRY_CATCH_BAD_ALLOC(hash_map_variant().visit(
                [&](auto& hash_map_with_key) { build_hash_map_with_not_founds(*hash_map_with_key, chunk_size); }));
    }
...
Status StreamAggregator::_output_changes(HashMapWithKey& hash_map_with_key, int32_t chunk_size,
                                         vectorized::ChunkPtr* result_chunk, vectorized::ChunkPtr* intermediate_chunk,
                                         std::vector<vectorized::ChunkPtr>& detail_chunks) {
...
Status StreamAggregator::_output_result_post_retract_changes(
        size_t chunk_size, const vectorized::Columns& group_by_columns, const vectorized::Column* count_column,
        const vectorized::Buffer<uint8_t>& prev_existence, const ChunkPtr& prev_result_chunk, ChunkPtr* result_chunk) {
    ...
    auto post_chunk_result =
            _build_output_chunk_with_ops(post_group_by_columns, post_agg_result_columns, post_ops, false);
...
Status StreamAggregator::_output_result_prev_retract_changes(size_t chunk_size,
    ...
    if (prev_count == 0) {
        ...
        *prev_result_chunk =
                _build_output_chunk_with_ops(prev_group_by_columns, prev_agg_result_columns, prev_ops, false);
    } else {
