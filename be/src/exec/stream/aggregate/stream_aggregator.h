...
private:
    ...
    Status _output_intermediate_changes(int32_t chunk_size, const vectorized::Columns& group_by_columns,
    ...
    Status _output_result_changes(int32_t chunk_size, const vectorized::Columns& group_by_columns,
                                  vectorized::ChunkPtr* intermediate_chunk);

    // Compute agg's count for each group by keys which are used to generate different Ops.
    Status _output_post_count_column(size_t chunk_size, vectorized::Column* agg_count_column);
    ...
    // @return
    // prev_result_chunk: chunk which is generated by prev results to output
    // pre_existence    : represents the existence for each row in the chunk.
    Status _output_result_prev_retract_changes(size_t chunk_size, const vectorized::Columns& group_by_columns,
                                               const vectorized::Column* agg_count_column, ChunkPtr* prev_result_chunk,
                                               vectorized::Buffer<uint8_t>* prev_existence);
    // Output incremental(post) output based on previous existence and final count:
    // If prev output exists and final count > 0: generate UPDATE_AFTER
    // If prev output not exists and final count > 0: generate INSERT
    Status _output_result_post_retract_changes(size_t chunk_size, const vectorized::Columns& group_by_columns,
                                               const vectorized::Column* agg_count_column,
                                               const vectorized::Buffer<uint8_t>& prev_existence,
                                               const ChunkPtr& prev_result_chunk, ChunkPtr* result_chunk);
