...
public:
    ...
    virtual ChunkPtrOr seek_key(const DatumRow& key) = 0;
    ...
    virtual std::vector<ChunkPtrOr> seek_keys(const std::vector<DatumRow>& keys) = 0;
    ...
    virtual ChunkIteratorPtrOr prefix_scan_key(const DatumRow& key) = 0;

    // The batch api of `prefix_scan_key`.
    // NOTE: The count of the result of `seek_keys` must be exactly same to the input keys' count.
    virtual std::vector<ChunkIteratorPtrOr> prefix_scan_keys(const std::vector<DatumRow>& keys) = 0;
