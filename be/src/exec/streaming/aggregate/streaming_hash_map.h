...
public:
    ...
    Status compute_agg_states(size_t chunk_size, const Columns& key_columns, Buffer<AggDataPtr>* agg_states);
    // Get key from map, must only seek from cache only.
    AggStateValue get(AggStateKey key);
    // Called by each epoch, clean up some states generated by the epoch.
    void reset_by_epoch();
...
private:
    ...
    AggStateKey get_serialized_key(size_t i) { return {_keys_buffer + i * _max_keys_size, _keys_slice_sizes[i]}; }
