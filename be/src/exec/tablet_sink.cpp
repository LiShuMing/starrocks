    ...
    for (int i = 0; i < _output_tuple_desc->slots().size(); ++i) {
        ...
        if (desc->is_nullable() && !column_ptr->is_nullable()) {
            MutableColumnPtr new_column =
                    NullableColumn::create(std::move(column_ptr)->assume_mutable(), NullColumn::create(num_rows, 0));
    ...
    for (auto desc : _output_tuple_desc->slots()) {
        if (desc->type().type == TYPE_CHAR) {
            ...
            if (desc->is_nullable()) {
                ...
                MutableColumnPtr new_column =
                        NullableColumn::create(std::move(new_binary), nullable_column->null_column()->assume_mutable());
                chunk->update_column(std::move(new_column), desc->id());
            } else {
