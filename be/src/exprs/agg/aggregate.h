...
public:
    ...
    // eg, min(y) group by x, the input(x, y) is (1, 2), (1, 3), (1, 3), then for key x=1, the detail state is:
    // k, v
    // 2, 1
    // 3, 2
    // so when (1, 2) record is retracted, the new result (1, 3) will be generated by using the detail state.
    // Restore previous input for detail kind from the detail state table, `columns` have two columns:
    ...
    // Output need sync group by keys.
    virtual void output_detail_sync(FunctionContext* ctx, size_t chunk_size, Column* to,
                                    AggDataPtr __restrict state) const {
        throw std::runtime_error("restore_detail function in aggregate is not supported for now.");
    }
    ...
    // Implement this function to indicate whether to sync detail state(iter the state map) to genreate the
    // final result.
    virtual void sync_detail(FunctionContext* ctx, AggDataPtr __restrict state, size_t chunk_size,
    ...
    // Columns `tos` contain two columns:
