...
public:
    ...
    void serialize_to_column(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* to) const override {
        ...
        if (map_column->keys_column()->is_nullable()) {
            // Key could not be NULL.
            auto nullable_column = map_column->keys_column();
            down_cast<NullableColumn*>(nullable_column.get())
                    ->null_column_data()
                    .resize(down_cast<NullableColumn*>(nullable_column.get())->null_column_data().size() + elem_size);
        }
    ...
    void convert_to_serialize_format(FunctionContext* ctx, const Columns& src, size_t chunk_size,
        ...
        auto offsets_col = column->offsets_column();
