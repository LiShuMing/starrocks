...
private:
    ...
    static void _build_hash_table(const ColumnPtr& column, ArrayContainsState* state) {
        ...
        const auto& elements_column = down_cast<const NullableColumn*>(array_column->elements_column().get())->data_column();
        const auto& null_column = down_cast<const NullableColumn*>(array_column->elements_column().get())->null_column();
