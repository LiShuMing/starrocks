...
StatusOr<ColumnPtr> ArrayMapExpr::evaluate_lambda_expr(ExprContext* context, Chunk* chunk,
    ...
    for (int i = 0; i < argument_num; ++i) {
        ...
        UInt32Column::MutablePtr offsets_column =
                UInt32Column::static_pointer_cast(array_column->offsets_column()->as_mutable_ptr());
        ...
        if (input_elements[i]->is_constant()) {
            ...
            elements_column = down_cast<ArrayColumn*>(data_column.get())->elements_column();
        }
    ...
    if constexpr (independent_lambda_expr) {
        ...
        if (all_const_input && capture_slot_ids.empty()) {
            ...
            auto aligned =
                    ColumnHelper::align_return_type(std::move(unpacked), type().children[0], unpacked->size(), true);
            column = Column::mutate(aligned);
        } else {
