    ...
    StatusOr<ColumnPtr> evaluate_checked(ExprContext* context, Chunk* ptr) override {
        ...
        if constexpr (FromType == TYPE_JSON || ToType == TYPE_JSON) {
            if constexpr (lt_is_decimal<FromType>) {
                ...
                result_column = Column::mutate(
                        CastFn<TYPE_DOUBLE, TYPE_JSON, AllowThrowException>::cast_fn(std::move(double_column)));
            } else {
                result_column =
                        Column::mutate(CastFn<FromType, ToType, AllowThrowException>::cast_fn(std::move(column)));
            }
