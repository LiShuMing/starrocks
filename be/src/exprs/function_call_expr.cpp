...
const FunctionDescriptor* VectorizedFunctionCallExpr::_get_function(const TFunction& fn,
    ...
    if (fn.__isset.agg_state_desc) {
        ...
        if (func_name.ends_with("_state")) {
            ...
            auto execute_func = std::bind(&AggStateBaseFunction::execute, _agg_state_func.get(), std::placeholders::_1,
                                          std::placeholders::_2);
            auto prepare_func = std::bind(&AggStateBaseFunction::prepare, _agg_state_func.get(), std::placeholders::_1,
                                          std::placeholders::_2);
            auto close_func = std::bind(&AggStateBaseFunction::close, _agg_state_func.get(), std::placeholders::_1,
                                        std::placeholders::_2);
            ...
            auto agg_state_desc = AggStateDesc::from_thrift(fn.agg_state_desc);
            _agg_state_func =
                    std::make_shared<AggStateMergeFunction>(agg_state_desc, return_type, std::move(arg_nullables));
            auto execute_func = std::bind(&AggStateBaseFunction::execute, _agg_state_func.get(), std::placeholders::_1,
                                          std::placeholders::_2);
            auto prepare_func = std::bind(&AggStateBaseFunction::prepare, _agg_state_func.get(), std::placeholders::_1,
                                          std::placeholders::_2);
            ...
            auto agg_state_desc = AggStateDesc::from_thrift(fn.agg_state_desc);
            _agg_state_func =
                    std::make_shared<AggStateUnionFunction>(agg_state_desc, return_type, std::move(arg_nullables));
            auto execute_func = std::bind(&AggStateBaseFunction::execute, _agg_state_func.get(), std::placeholders::_1,
                                          std::placeholders::_2);
            auto prepare_func = std::bind(&AggStateBaseFunction::prepare, _agg_state_func.get(), std::placeholders::_1,
                                          std::placeholders::_2);
