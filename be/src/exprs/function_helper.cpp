...
NullColumnMutablePtr FunctionHelper::union_nullable_column(const ColumnPtr& v1, const ColumnPtr& v2) {
...
NullColumnMutablePtr FunctionHelper::union_null_column(const NullColumnPtr& v1, const NullColumnPtr& v2) {
    // union null column
    auto null1_begin = (uint8_t*)v1->get_data().data();
    auto null2_begin = (uint8_t*)v2->get_data().data();

    const size_t row_num = v1->size();
    NullColumnMutablePtr null_result = NullColumn::create();
