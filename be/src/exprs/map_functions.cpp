...
StatusOr<ColumnPtr> MapFunctions::map_from_arrays(FunctionContext* context, const Columns& columns) {
    ...
    if (keys_column->is_nullable()) {
        ...
        keys_null = keys->immutable_null_column();
    ...
    if (values_column->is_nullable()) {
        ...
        values_null = values->immutable_null_column();
...
static inline std::tuple<NullColumn::MutablePtr, Column*, Column*, const UInt32Column*> unpack_map_column(
        const ColumnPtr& input) {
    NullColumn::MutablePtr map_null = nullptr;
    ...
    if (map->is_nullable()) {
        ...
        map_col = down_cast<MapColumn*>(nullable->mutable_data_column());
...
StatusOr<ColumnPtr> MapFunctions::map_concat(FunctionContext* context, const Columns& columns) {
    ...
    NullColumn::MutablePtr all_nulls[columns_num];
