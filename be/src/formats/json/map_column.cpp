...
Status add_map_column(Column* column, const TypeDescriptor& type_desc, const std::string& name,
    ...
    try {
        ...
        auto keys = map_column->keys_column();
        auto values = map_column->values_column();
        auto offsets = map_column->offsets_column();
        for (auto field : obj) {
            {
                ...
                RETURN_IF_ERROR(add_nullable_column(keys.get(), type_desc.children[0], name, &field_key, true));
            }

            {
                simdjson::ondemand::value field_value = field.value();
                RETURN_IF_ERROR(add_nullable_column(values.get(), type_desc.children[1], name, &field_value, true));
            }
            ++field_count;
        }
        down_cast<UInt32Column*>(offsets.get())
                ->append(down_cast<UInt32Column*>(offsets.get())->immutable_data().back() + field_count);
