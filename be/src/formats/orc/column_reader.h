...
private:
    ...
    Status _fill_int_column_with_null_from_cvb(OrcColumnVectorBatch* data, MutableColumnPtr& col, size_t from,
                                               size_t size);
...
private:
    template <typename T>
    inline void _fill_decimal_column_generic(orc::ColumnVectorBatch* cvb, MutableColumnPtr& col, size_t from,
                                             size_t size);
};
