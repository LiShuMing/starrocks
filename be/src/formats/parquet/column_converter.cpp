...
public:
    ...
    Status convert(const Column* src, Column* dst) override {
        ...
        auto* dst_column =
                ColumnHelper::as_raw_column<FixedLengthColumn<DestType>>(dst_nullable_column->mutable_data_column());
...
public:
    ...
    Status convert(const Column* src, Column* dst) override {
        ...
        auto& dst_data = dst_column->get_data();
        const auto& src_null_data_const = src_nullable_column->immutable_null_column_data();
        auto& dst_null_data = dst_nullable_column->null_column_data();
...
#define M(SZ, K, T)                                                                                                  \
    case SZ:                                                                                                         \
        if (has_null) {                                                                                              \
            t_convert<SZ, K, T, true>(size, dst_null_data.data(), const_cast<uint8_t*>(src_null_data_const.data()),  \
                                      dst_data.data(), src_data.data());                                             \
        } else {                                                                                                     \
            t_convert<SZ, K, T, false>(size, dst_null_data.data(), const_cast<uint8_t*>(src_null_data_const.data()), \
                                       dst_data.data(), src_data.data());                                            \
        }                                                                                                            \
        break;
