...
AggStateDesc AggStateDesc::from_thrift(const TAggStateDesc& desc) {
    ...
    // return type
    auto return_type = from_thrift(desc.ret_types);
    // arg types
...
TypeDescriptor AggStateDesc::from_thrift(const std::vector<TTypeNode>& types) {
    int idx = 0;
    TypeDescriptor result(types, &idx);
    DCHECK_EQ(idx, types.size());
    return result;
}
...
AggStateDesc AggStateDesc::from_protobuf(const AggStateDescPB& desc) {
    ...
    // ret type
    auto ret_type = from_protobuf(desc.ret_type());
...
TypeDescriptor AggStateDesc::from_protobuf(const PTypeDesc& ltype) {
    int idx = 0;
...
void AggStateDesc::thrift_to_protobuf(const TAggStateDesc& desc, AggStateDescPB* pb) {
    ...
    // ret type
    auto ret_type_desc = from_thrift(desc.ret_types);
