...
Status UpdateManager::_read_chunk_for_upsert(const TxnLogPB_OpWrite& op_write, const TabletSchemaCSPtr& tschema,
...

    {
        ...
        for (uint32_t cid : update_cids) {
            ...
            auto mut_col = full_chunk->get_column_by_id(cid)->as_mutable_ptr();
            RETURN_IF_ERROR(col_iter->fetch_values_by_rowid(insert_rowids.data(), insert_rowids.size(), mut_col.get()));
        }
    }
    ...
    for (uint32_t cid = 0; cid < tschema->num_columns(); ++cid) {
        ...
        if (has_default_value) {
            ...
            auto mut_col = full_chunk->get_column_by_id(cid)->as_mutable_ptr();
            RETURN_IF_ERROR(default_value_iter->fetch_values_by_rowid(nullptr, insert_rowids.size(), mut_col.get()));
        } else {
            auto mut_col = full_chunk->get_column_by_id(cid)->as_mutable_ptr();
