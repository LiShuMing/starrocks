...
Status StructColumnIterator::fetch_subfield_by_rowid(const rowid_t* rowids, size_t size, Column* values) {
    ...
    for (int i = 0; i < _access_iters.size(); ++i) {
        if (fields[i]->is_constant()) {
            // Field is a const column: materialize its data column before reading.
            auto* const_column = down_cast<ConstColumn*>(fields[i].get());
            auto* data_col = const_column->mutable_data_column();
            fields[i] = data_col->as_mutable_ptr();
