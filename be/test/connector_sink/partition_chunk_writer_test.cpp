...
public:
    ...
    int64_t commit() {
        ...
        _result_chunks.emplace_back(result_chunk);
...
TEST_F(PartitionChunkWriterTest, buffer_partition_chunk_writer) {
    ...
    auto writer_helper = WriterHelper::instance();
    {
        ...
        chunk->get_mutable_column_by_index(0)->append_datum(Slice("aaa"));
...
TEST_F(PartitionChunkWriterTest, sort_column_asc) {
    ...
    // Normal write and flush to file
    {
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            chunk->get_mutable_column_by_index(0)->append_datum(Slice("ccc" + suffix));
            chunk->get_mutable_column_by_index(0)->append_datum(Slice("bbb" + suffix));
            chunk->get_mutable_column_by_index(0)->append_datum(Slice("aaa" + suffix));
...
TEST_F(PartitionChunkWriterTest, sort_multiple_columns) {
    ...
    // Write and spill multiple chunks
    {
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            std::string suffix = std::to_string(3 - i);
            chunk->get_mutable_column_by_index(0)->append_datum(Slice("ccc" + suffix));
            chunk->get_mutable_column_by_index(1)->append_datum(Slice("222" + suffix));
            chunk->get_mutable_column_by_index(0)->append_datum(Slice("ccc" + suffix));
            chunk->get_mutable_column_by_index(1)->append_datum(Slice("111" + suffix));
            chunk->get_mutable_column_by_index(0)->append_datum(Slice("bbb" + suffix));
            chunk->get_mutable_column_by_index(1)->append_datum(Slice("222" + suffix));
            chunk->get_mutable_column_by_index(0)->append_datum(Slice("bbb" + suffix));
            chunk->get_mutable_column_by_index(1)->append_datum(Slice("111" + suffix));
