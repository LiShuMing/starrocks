...
struct ColumnRandomAppender {
    static bool append(MutableColumnPtr col, int sz) {
...
struct FakeChunks {
    FakeChunks(ObjectPool* pool, std::vector<TypeDescriptor*> descs, std::vector<BuildOptions> build_options) {
        ...
        for (int i = 0; i < _type_descs.size(); ++i) {
            _slot_refs.emplace_back(pool->add(new ColumnRef(*_type_descs[i], i)));
        }
    ...
    ChunkPtr next_chunk(int chunk_sz) {
        ...
        for (int i = 0; i < _slot_refs.size(); ++i) {
            columns.emplace_back(ColumnHelper::create_column(*_type_descs[i], _build_options[i].is_nullable_value));
        }
        for (int i = 0; i < _slot_refs.size(); ++i) {
            ...
            if (build_option.use_random_value) {
                if (!build_option.use_list_values.empty()) {
                    ...
                    dispatch_function<ColumnRandomAppender>(_type_descs[i]->type, std::move(columns[i]), chunk_sz);
                }
            } else {
        ...
        return std::make_shared<Chunk>(std::move(columns), map);
    }
