...
public :
    ...
    void RunBatchAndCheck(
            size_t run_id,
            const StreamRowData& input_rows,
            const StreamRowData& expect_result_data) {
        return RunBatchAndCheck(run_id, input_rows, expect_result_data,
                                StreamRowData{{}, {}});
    }
    ...
    void RunBatchAndCheck(
        ...
        DCHECK_IF_ERROR(_stream_aggregator->output_changes(chunk_size, &result_chunk_ptr,
                                                           &intermediate_chunk_ptr,
                                                           detail_chunks));
        for (auto& column: result_chunk_ptr->columns()) {
            std::cout << "[RunBatchAndCheck] result column:" << column->debug_string() << std::endl;
        }
        CheckChunk(result_chunk_ptr, get_slot_types(_slot_infos[2]),
                   expect_result_data.rows, expect_result_data.ops);
        ...
        if (!expect_intermediate_data.rows.empty()) {
            ...
            CheckChunk(intermediate_chunk_ptr, get_slot_types(_slot_infos[1]),
                       expect_intermediate_data.rows, expect_intermediate_data.ops);
        }
...
public:
    CountStreamAggregateTestBase(bool is_generate_retract) {
        ...
        _agg_infos = std::vector<AggInfo>{
                // slot_index, agg_name, agg_intermediate_type, agg_result_type
        ...
        _stream_aggregator = _create_stream_aggregator(_slot_infos, _group_by_infos,
                                                       _agg_infos, is_generate_retract, 0);
    }
};
...
TEST_F(CountStreamAggregateTest, TestNoRetracts_NoGenerateRetracts) {
    ...
    // 2 1 0
    RunBatchAndCheck(1,
                     StreamRowData{{{1, 2, 1}, {1, 2, 2}}, {0, 0, 0}},
                     StreamRowData{{{1, 2}, {2, 1}}, {0, 0}});
...
TEST_F(CountStreamAggregateTest, TestWithRetracts_NoGenerateRetracts) {
    ...
    // 2 1 0
    RunBatchAndCheck(1,
                     StreamRowData{{{1, 2, 1}, {1, 2, 2}}, {0, 0, 1}},
                     StreamRowData{{{1, 2}, {0, 1}}, {0, 0}});
    _stream_aggregator->close(_state);
}
...
TEST_F(CountStreamAggregateTest, TestNoRetracts_MultiRuns) {
    ...
    // 3 1 0
    RunBatchAndCheck(3,
                     StreamRowData{{{1, 2, 3}, {1, 2, 2}}, {0, 0, 0}},
...
TEST_F(CountStreamAggregateTestWithGenerateRetracts, TestWithRetracts_MultiRuns) {
    ...
    // 3 1 INSERT
    RunBatchAndCheck(2,
                     StreamRowData{{{2, 1, 3}, {2, 1, 1}}, {1, 0, 0}},
    ...
    // 3 1 UPDATE_AFTER
    RunBatchAndCheck(3,
                     StreamRowData{{{2, 1, 3}, {2, 1, 1}}, {0, 0, 0}},
                     StreamRowData{{{1, 3, 2, 1, 3}, {1, 1, 1, 2, 2}}, {2, 2, 0, 3, 3}}); // make sure state table is also cleaned.
...
public:
    MinMaxCountStreamAggregateTest(bool is_generate_retract) {
        ...
        _agg_infos = std::vector<AggInfo>{
                // slot_index, agg_name, agg_intermediate_type, agg_result_type
                {1, "min", TYPE_BIGINT, TYPE_BIGINT},
                {1, "max", TYPE_BIGINT, TYPE_BIGINT},
                {1, "count", TYPE_BIGINT, TYPE_BIGINT}
        };
        ...
        _state->set_desc_tbl(_tbl);
        _stream_aggregator = _create_stream_aggregator(_slot_infos, _group_by_infos,
                                                       _agg_infos, is_generate_retract,
                                                       2);
    }
};
...
TEST_F(MinMaxCountStreamAggregateTestWithoutRetract, TestNoRetracts_MultiRun) {
    ...
    // Run 2
    RunBatchAndCheck(2,
                     StreamRowData{{{1, 2, 3}, {1, 3, 3}}, {0, 0, 0}},
                     StreamRowData{{{1, 2, 3}, {1, 2, 3}, {2, 3, 3}, {3, 2, 1}}, {0, 0, 0}});
    // Run 3
    RunBatchAndCheck(3,
                     StreamRowData{{{2, 3}, {1, 4}}, {0, 0}},
...
TEST_F(MinMaxCountStreamAggregateTestWithRetract, TestWihRetracts_MultiRun_NoRetractInput) {
    ...
    // 2    2   2   1   0
    RunBatchAndCheck(1,
                     StreamRowData{{{1, 2, 1}, {0, 2, 2}}, {0, 0, 0}},
