...
class StreamSinkOperator final : public pipeline::Operator {
public:
    StreamSinkOperator(OperatorFactory* factory, int32_t id, int32_t plan_node_id, int32_t driver_sequence)
            : Operator(factory, id, "stream_sink", plan_node_id, driver_sequence) {}

    ~StreamSinkOperator() override = default;

    bool has_output() const override { return _cur_output != nullptr; }
    ...
    StatusOr<vectorized::ChunkPtr> pull_chunk(RuntimeState* state) override {
        if (_cur_output) {
            VLOG_ROW << "[StreamSinkOperator] result:" << _cur_output->debug_string();
        }

        auto output_chunk = std::move(_cur_output);
        _cur_output = nullptr;
    ...
    Status push_chunk(RuntimeState* state, const vectorized::ChunkPtr& chunk) override {
        ...
        this->_cur_output = chunk;
        return Status::OK();
    }

private:
    bool _is_finished = false;
    ChunkPtr _cur_output;
};

class StreamSinkOperatorFactory final : public OperatorFactory {
public:
    StreamSinkOperatorFactory(int32_t id, int32_t plan_node_id)
            : OperatorFactory(id, "stream_sink_factory", plan_node_id) {}

    ~StreamSinkOperatorFactory() override = default;

    pipeline::OperatorPtr create(int32_t degree_of_parallelism, int32_t driver_sequence) override {
        return std::make_shared<StreamSinkOperator>(this, _id, _plan_node_id, driver_sequence);
    }
