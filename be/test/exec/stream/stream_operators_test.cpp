...
//      _pipeline_builder = [&](RuntimeState* state) {
//        OpFactories op_factories;
//        auto source_factory = std::make_shared<TestStreamSourceOperatorFactory>(next_operator_id(), next_plan_node_id(),
...
//        op_factories.emplace_back(std::make_shared<TestStreamSinkOperatorFactory>(next_operator_id(), next_plan_node_id()));
//        _pipelines.push_back(std::make_shared<pipeline::Pipeline>(next_pipeline_id(), op_factories));
//      };
//      return Status::OK();
//    }));
...
//            .epoch_id=0,
//            .trigger_mode=TriggerMode::kManualTrigger
//    };
//    DCHECK_IF_ERROR(StartEpoch(epoch_info));
//    DCHECK_IF_ERROR(WaitUntilEpochEnd(epoch_info));
//
