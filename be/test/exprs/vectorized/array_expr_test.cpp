...
TEST_F(ArrayExprTest, test_evaluate) {
    ...
    {
        std::unique_ptr<Expr> expr(ExprsTestHelper::create_array_expr(type_arr_int));
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_int32_column({1, 3, 6}), PrimitiveType::TYPE_INT));
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_int32_column({2, 4, 8}), PrimitiveType::TYPE_INT));
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_int32_column({4, 8, 12}), PrimitiveType::TYPE_INT));
        auto result = expr->evaluate(nullptr, nullptr);
    ...
    // [6, 8, 12]
    {
        ...
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_int32_column({1, 3, 6}), PrimitiveType::TYPE_INT));
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_int32_column({2, 4, 8}), PrimitiveType::TYPE_INT));
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_int32_column({4, 0, 12}, {0, 1, 0}), PrimitiveType::TYPE_INT));
        auto result = expr->evaluate(nullptr, nullptr);
        EXPECT_EQ(3, result->size());
    ...
    // ["", "xyz", "x"]
    {
        ...
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_string_column({"a", "ab", ""}), type_varchar));
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_string_column({"", "bcd", "xyz"}), type_varchar));
        expr->add_child(new_mock_expr(ColumnTestBuilder::build_string_column({"x", "", "x"}, {0, 1, 0}), type_varchar));
        auto result = expr->evaluate(nullptr, nullptr);
        EXPECT_EQ(3, result->size());
