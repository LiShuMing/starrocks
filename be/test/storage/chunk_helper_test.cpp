...
SegmentedChunkPtr ChunkHelperTest::build_segmented_chunk() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        for (int j = 0; j < chunk_rows; j++) {
            chunk->get_mutable_column_by_index(0)->append_datum(row_id++);
            std::string str = fmt::format("str{}", row_id);
            chunk->get_mutable_column_by_index(1)->append_datum(Slice(str));
        }
...
TEST_F(ChunkHelperTest, Accumulator) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        chunk->get_mutable_column_by_index(0)->append_default(1025);
        input_rows += 1025;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        chunk->get_mutable_column_by_index(0)->append_default(8888);
        input_rows += 8888;
        static_cast<void>(accumulator.push(std::move(chunk)));
    }
