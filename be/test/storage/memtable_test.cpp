...
TEST_F(MemTableTest, testPrimaryKeysWithDeletes) {
    ...
    for (int i = 0; i < n; i++) {
        Datum v;
        v.set_int64(i);
        chunk->get_mutable_column_by_index(0)->append_datum(v);
        v.set_int32(i * 3);
        chunk->get_mutable_column_by_index(1)->append_datum(v);
        v.set_int8(i % 5 == 0 ? TOpType::DELETE : TOpType::UPSERT);
        chunk->get_mutable_column_by_index(2)->append_datum(v);
    }
    vector<uint32_t> indexes;
    indexes.reserve(n);
    for (int i = 0; i < n; i++) {
...
TEST_F(MemTableTest, testPrimaryKeysNullableSortKey) {
    ...
    shared_ptr<Chunk> chunk = ChunkHelper::new_chunk(*_slots, n);
    for (int i = 0; i < n; i++) {
        chunk->get_mutable_column_by_index(0)->append_datum(Datum(static_cast<int64_t>(i)));
        chunk->get_mutable_column_by_index(1)->append_datum(Datum(static_cast<int32_t>(n - 1 - i)));
        if (i % 2) {
            chunk->get_mutable_column_by_index(2)->append_datum(Datum(static_cast<int8_t>(i)));
        } else {
            chunk->get_mutable_column_by_index(2)->append_nulls(1);
        }
    }
    vector<uint32_t> indexes;
    indexes.reserve(n);
    ...
    for (int i = 0; i < n / 2; i++) {
        expected_chunk->get_mutable_column_by_index(0)->append_datum(Datum(static_cast<int64_t>(2 * i)));
        expected_chunk->get_mutable_column_by_index(1)->append_datum(Datum(static_cast<int32_t>(n - 1 - 2 * i)));
        expected_chunk->get_mutable_column_by_index(2)->append_nulls(1);
    }

    for (int i = 0; i < n / 2; i++) {
        expected_chunk->get_mutable_column_by_index(0)->append_datum(Datum(static_cast<int64_t>(2 * i + 1)));
        expected_chunk->get_mutable_column_by_index(1)->append_datum(Datum(static_cast<int32_t>(n - 2 - 2 * i)));
        expected_chunk->get_mutable_column_by_index(2)->append_datum(Datum(static_cast<int8_t>(2 * i + 1)));
    }
...
TEST_F(MemTableTest, testPrimaryKeysSizeLimitCompositePK) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        chunk->get_mutable_column_by_index(2)->append_datum(v);
        v.set_uint8(1);
        chunk->get_mutable_column_by_index(3)->append_datum(v);
        v.set_int32(i * 3);
        chunk->get_mutable_column_by_index(4)->append_datum(v);
        v.set_int8(i % 5 == 0 ? TOpType::DELETE : TOpType::UPSERT);
        chunk->get_mutable_column_by_index(5)->append_datum(v);
    }
    vector<uint32_t> indexes;
    indexes.reserve(n);
    for (int i = 0; i < n; i++) {
