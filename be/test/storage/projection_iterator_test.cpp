...
public:
    ...
    Status do_get_next(Chunk* chunk) override {
        ...
        for (size_t i = 0; i < n; i++) {
            chunk->get_mutable_column_by_index(0)->append_datum(Datum(_c1[_idx]));
            chunk->get_mutable_column_by_index(1)->append_datum(Datum(Slice(_c2[_idx])));
            chunk->get_mutable_column_by_index(2)->append_datum(Datum(_c3[_idx]));
            _idx++;
        }
