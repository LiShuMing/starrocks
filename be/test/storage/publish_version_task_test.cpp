...
public:
    ...
    static void rowset_writer_add_rows(std::unique_ptr<RowsetWriter>& writer, const TabletSchemaCSPtr& tablet_schema) {
        ...
        for (size_t i = 0; i < 1024; ++i) {
            ...
            // auto& cols = chunk->columns();  // Use get_mutable_column_by_index instead
            chunk->get_mutable_column_by_index(0)->append_datum(Datum(static_cast<int32_t>(i)));
            Slice field_1(test_data[i]);
            chunk->get_mutable_column_by_index(1)->append_datum(Datum(field_1));
            chunk->get_mutable_column_by_index(2)->append_datum(Datum(static_cast<int32_t>(10000 + i)));
        }
...
TEST_F(PublishVersionTaskTest, test_publish_version_overwrite_failed) {
    ...
    writer_options.slots = &tuple_desc->slots();
    {
        ...
        for (size_t i = 0; i < 8; ++i) {
            ...
            Slice s(s_str);
            chunk->get_mutable_column_by_index(1)->append_datum(Datum(s));
            chunk->get_mutable_column_by_index(2)->append_datum(Datum(static_cast<int32_t>(i)));
        }
