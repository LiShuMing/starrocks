...
public:
    ...
    void build_multi_get_request(DatumTuple& tuple, Chunk* key_chunk, std::vector<bool>& found, Chunk* value_chunk) {
        key_chunk->get_mutable_column_by_index(0)->append_datum(tuple.get(0));
        key_chunk->get_mutable_column_by_index(1)->append_datum(tuple.get(1));
        key_chunk->get_mutable_column_by_index(2)->append_datum(tuple.get(2));
        found.emplace_back(true);
        value_chunk->get_mutable_column_by_index(0)->append_datum(tuple.get(3));
        value_chunk->get_mutable_column_by_index(1)->append_datum(tuple.get(4));
    }
    ...
    void build_scan_request(DatumTupleVector& tuples, Schema& scan_key_schema, std::vector<int> tuple_index,
        ...
        for (int i : tuple_index) {
            ...
            for (int j = 0; j < value_column_index.size(); j++) {
                ...
                result->get_mutable_column_by_index(j)->append_datum(datum);
            }
        }
    }
...
TEST_F(TableReaderTest, test_basic_read) {
    ...
    // key (1, 1, 4) not found
    key_chunk->get_mutable_column_by_index(0)->append_datum(Datum((int64_t)1));
    key_chunk->get_mutable_column_by_index(1)->append_datum(Datum((int32_t)1));
    key_chunk->get_mutable_column_by_index(2)->append_datum(Datum((int32_t)4));
    ...
    // key (5, 8, 9) not found
    key_chunk->get_mutable_column_by_index(0)->append_datum(Datum((int64_t)5));
    key_chunk->get_mutable_column_by_index(1)->append_datum(Datum((int32_t)8));
    key_chunk->get_mutable_column_by_index(2)->append_datum(Datum((int32_t)9));
    expected_found.emplace_back(false);
