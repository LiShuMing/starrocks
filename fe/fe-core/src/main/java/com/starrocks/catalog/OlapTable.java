...
public class OlapTable extends Table {
    ...
    public Map<Long, MaterializedIndexMeta> getAssociatedIndexMetas() {
        Map<Long, MaterializedIndexMeta> associatedIndexMetas = Maps.newHashMap();
        for (Map.Entry<Long, MaterializedIndexMeta> entry : indexIdToMeta.entrySet()) {
            if (entry.getValue().isLogical()) {
                associatedIndexMetas.put(entry.getKey(), entry.getValue());
            }
        }
        return associatedIndexMetas;
    }
