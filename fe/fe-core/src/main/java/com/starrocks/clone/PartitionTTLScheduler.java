...
public class PartitionTTLScheduler {
    ...
    private boolean isValidRangePartitionTTL(Database db,
        ...
        if (partitionInfo instanceof RangePartitionInfo) {
            ...
            String recyclingPolicy = olapTable.getTableProperty().getPartitionRecyclingPolicy();
            if (Strings.isNullOrEmpty(recyclingPolicy)) {
    ...
    private List<String> getExpiredPartitionNames(Database db,
        ...
        try {
            if (partitionInfo instanceof RangePartitionInfo) {
                ...
                String recyclingPolicy = olapTable.getTableProperty().getPartitionRecyclingPolicy();
                dropPartitionNames = buildDropPartitionCauseByRecyclingPolicy(db, olapTable, recyclingPolicy);
            }
    ...
    private List<String> buildDropPartitionCauseByRecyclingPolicy(Database db,
                                                                  OlapTable olapTable,
                                                                  String recyclingPolicy) {
        ...
        Expr whereExpr = SqlParser.parseSqlToExpr(recyclingPolicy, SqlModeHelper.MODE_DEFAULT);
        if (whereExpr == null) {
            LOG.warn("database={}, table={} failed to parse recycling policy: {}", db.getFullName(), olapTable.getName(),
                    recyclingPolicy);
            return Lists.newArrayList();
        }
        List<String> dropPartitionNames = ListPartitionPruner.getPartitionNamesByExpr(olapTable, tableName, whereExpr, context);
        return dropPartitionNames;
    }
