...
import static com.starrocks.common.util.PropertyAnalyzer.PROPERTIES_PARTITION_RECYCLING_POLICY;
...
public class DynamicPartitionUtil {
    ...
    private static boolean isTTLPartitionTable(OlapTable olapTable,
        ...
        if (partitionInfo instanceof RangePartitionInfo) {
            ...
            // if ttl is not set in table property, return false
            Map<String, String> properties = tableProperty.getProperties();
            if (!properties.containsKey(PROPERTIES_PARTITION_RECYCLING_POLICY)) {
                return false;
            }
            return true;
        } else {
    ...
    private static boolean isEnabledTablePartitionTTL(OlapTable olapTable,
        ...
        if (partitionInfo instanceof RangePartitionInfo) {
            ...
            if (Strings.isNullOrEmpty(tableProperty.getPartitionRecyclingPolicy())) {
                return false;
            }
            return true;
        } else {
