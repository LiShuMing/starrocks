...
public class ExchangeNode extends PlanNode {
    ...
    public boolean pushDownRuntimeFilters(RuntimeFilterDescription description, Expr probeExpr, List<Expr> partitionByExprs) {
        ...
        List<Expr> newPartitionByExprs = partitionByExprs.isEmpty() ? dataPartition.getPartitionExprs() : partitionByExprs;
        boolean accept = pushCrossExchange(description, probeExpr, newPartitionByExprs);
        ...
        if (probeExpr.isBoundByTupleIds(getTupleIds())) {
            if (onExchangeNode || (description.isLocalApplicable() && description.inLocalFragmentInstance())) {
                ...
                description.addPartitionByExprs(id.asInt(), newPartitionByExprs);
    ...
    private boolean pushCrossExchange(RuntimeFilterDescription description, Expr probeExpr, List<Expr> partitionByExprs) {
        ...
        // If crossExchange is accepted(broadcast or only one partition by expr), no need to check its children anymore.
        List<Expr> toCheckPartitionByExprs = crossExchange ? Lists.newArrayList() : partitionByExprs;;

        boolean accept = false;
        description.enterExchangeNode();
        for (PlanNode node : children) {
            if (node.pushDownRuntimeFilters(description, probeExpr, toCheckPartitionByExprs)) {
