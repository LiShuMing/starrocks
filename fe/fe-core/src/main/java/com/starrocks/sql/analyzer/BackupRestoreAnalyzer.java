    ...
    public static class BackupRestoreStmtAnalyzerVisitor extends AstVisitor<Void, ConnectContext> {
        ...
        public Void visitBackupStatement(BackupStmt backupStmt, ConnectContext context) {
            ...
            if (!mvNameMVMap.isEmpty()) {
                ...
                for (Map.Entry<String, MaterializedView> e : mvNameMVMap.entrySet()) {
                    ...
                    for (BaseTableInfo baseTableInfo : mv.getBaseTableInfos()) {
                        if (!tableIdToTableRefMap.containsKey(baseTableInfo.getTableId())) {
                            throw new SemanticException(String.format("Backup/restore materialized view %s's" +
                ...
                Map<String, TableRef> newTableRefs = reorderTableRefsWithMaterializedView(tblPartsMap, mvNameMVMap,
        ...
        private Map<String, TableRef> reorderTableRefsWithMaterializedView(Map<String, TableRef> tblPartsMap,
            ...
            for (Map.Entry<String, TableRef> e : tblPartsMap.entrySet()) {
                collectTableRefAndDependencies(e.getKey(), e.getValue(), mvPartsMap, tableIdToTableRefMap,
                        orderedTableNameRefMap);
            }
            return orderedTableNameRefMap;
        }

        private void collectTableRefAndDependencies(String tableName,
                                                    TableRef tableRef,
                                                    Map<String, MaterializedView> mvPartsMap,
                                                    Map<Long, TableRef> tableIdToTableRefMap,
            ...
            if (mvPartsMap.containsKey(tableName)) {
                ...
                for (BaseTableInfo baseTableInfo : mv.getBaseTableInfos()) {
                    ...
                    collectTableRefAndDependencies(baseTable.getName(), tableIdToTableRefMap.get(baseTable.getId()),
                            mvPartsMap, tableIdToTableRefMap, result);
                }
            }
