    ...
    private class Visitor extends AstVisitor<Scope, Scope> {
        ...
        public Scope visitTable(TableRelation node, Scope outerScope) {
            ...
            List<Column> fullSchema =
                    node.isBinlogQuery() ? appendBinlogColumns(table.getFullSchema()) : table.getFullSchema();
            List<Column> baseSchema =
                    node.isBinlogQuery() ? appendBinlogColumns(table.getBaseSchema()) : table.getBaseSchema();
