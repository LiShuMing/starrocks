...
public class CreateMaterializedViewStmt extends DdlStmt {
    ...
    public Map<String, Expr> parseDefineExprWithoutAnalyze(String originalSql) throws AnalysisException {
        ...
        for (SelectListItem selectListItem : selectList.getItems()) {
            ...
            String name = MVUtils.getMVColumnName(selectListItemExpr.toString());
            ...
            if (selectListItemExpr instanceof FunctionCallExpr) {
                ...
                switch (functionName.toLowerCase()) {
                    ...
                    case FunctionSet.COUNT:
                        MVColumnItem item = buildAggColumnItem(functionCallExpr, slots);
    ...
    private int genColumnAndSetIntoStmt(SelectRelation selectRelation) {
        ...
        for (int i = 0; i < selectListItems.size(); ++i) {
            ...
            if (!isReplay && slots.size() == 0) {
                throw new SemanticException("The materialized view currently does not support const expr in select statement");
            }
            ...
            if (selectListItemExpr instanceof FunctionCallExpr
                ...
                MVColumnItem mvColumnItem = buildAggColumnItem(functionCallExpr, slots);
                ...
                MVColumnItem mvColumnItem = buildNonAggColumnItem(selectListItemExpr, slots);
                if (!mvColumnNameSet.add(mvColumnItem.getName())) {
                    ErrorReport.reportSemanticException(ErrorCode.ERR_DUP_FIELDNAME, mvColumnItem.getName());
                }
    ...
    private MVColumnItem buildNonAggColumnItem(Expr defineExpr, List<SlotRef> baseSlotRefs) throws SemanticException {
        Type type = defineExpr.getType();
        String columnName;
        if (defineExpr instanceof  SlotRef) {
            columnName = ((SlotRef) defineExpr).getColumnName().toLowerCase();
        } else {
            //            throw new SemanticException("Create materialized view don't support non-slot ref expression:" +
            //                    defineExpr.childrenToSql());
            columnName = defineExpr.toSql();
        }
    ...
    private MVColumnItem buildAggColumnItem(FunctionCallExpr functionCallExpr, List<SlotRef> baseSlotRefs) {
        String functionName = functionCallExpr.getFnName().getFunction();
        ...
        Set<String> baseColumnNames = baseSlotRefs.stream().map(slot -> slot.getColumnName().toLowerCase()).
                collect(Collectors.toSet());
        String mvColumnName = MVUtils.getMVAggColumnName(functionName, baseColumName);
