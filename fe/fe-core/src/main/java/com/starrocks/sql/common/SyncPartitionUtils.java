...
public class SyncPartitionUtils {
    ...
    public static Range<PartitionKey> convertToDatePartitionRange(Range<PartitionKey> range) {
        PartitionKey lowerPartitionKey = convertPartitionKeyToDate(range.lowerEndpoint());
        PartitionKey upperPartitionKey = convertPartitionKeyToDate(range.upperEndpoint());
        return Range.closedOpen(lowerPartitionKey, upperPartitionKey);
    }

    private static PartitionKey convertPartitionKeyToDate(PartitionKey partitionKey) {
        LiteralExpr literalExpr = partitionKey.getKeys().get(0);
        if (!(literalExpr instanceof StringLiteral)) {
            return partitionKey;
        }
        LocalDateTime localDateTime = DateUtils.parseStrictDateTime(literalExpr.getStringValue());
        try {
            PartitionKey newPartitionKey = new PartitionKey();
            newPartitionKey.pushColumn(new DateLiteral(localDateTime, Type.DATE), PrimitiveType.DATE);
