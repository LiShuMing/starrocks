...
public class Group {
    ...
    private final Map<PhysicalPropertySet, CostContext> lowestCostExpressions;
    ...
    public PhysicalPropertySet updateOutputPropertySet(CostContext costContext, PhysicalPropertySet requiredPropertySet) {
        for (Map.Entry<PhysicalPropertySet, CostContext> entry : lowestCostExpressions.entrySet()) {
            if (entry.getKey().equals(requiredPropertySet)) {
                if (costContext.isBetterThan(entry.getValue())) {
                    lowestCostExpressions.put(requiredPropertySet, costContext);
    ...
    public void setBestExpression(CostContext costContext, PhysicalPropertySet physicalPropertySet) {
        if (lowestCostExpressions.containsKey(physicalPropertySet)) {
            if (costContext.isBetterThan(lowestCostExpressions.get(physicalPropertySet))) {
                lowestCostExpressions.put(physicalPropertySet, costContext);
            }
        } else {
            lowestCostExpressions.put(physicalPropertySet, costContext);
        }
    ...
    public void setBestExpressionWithStatistics(CostContext costContext,
    ...
    public void replaceBestExpressionProperty(PhysicalPropertySet oldProperty, PhysicalPropertySet newProperty,
                                              double cost) {
        CostContext lowestExpression = lowestCostExpressions.get(oldProperty);
    ...
    public void replaceBestExpression(GroupExpression oldGroupExpression, GroupExpression newGroupExpression) {
        Map<PhysicalPropertySet, CostContext> needReplaceBestExpressions = Maps.newHashMap();
        for (Iterator<Map.Entry<PhysicalPropertySet, CostContext>> iterator =
                lowestCostExpressions.entrySet().iterator(); iterator.hasNext(); ) {
            Map.Entry<PhysicalPropertySet, CostContext> entry = iterator.next();
            CostContext costContext = entry.getValue();
            if (costContext.getGroupExpression().equals(oldGroupExpression)) {
                needReplaceBestExpressions.put(entry.getKey(), new CostContext(newGroupExpression, costContext.getCost()));
                iterator.remove();
            }
        }
    ...
    public Collection<CostContext> getAllBestExpressionWithCost() {
    ...
    public void mergeGroup(Group other) {
        ...
        for (Map.Entry<PhysicalPropertySet, CostContext> entry : other.lowestCostExpressions
                .entrySet()) {
            ...
            setBestExpressionWithStatistics(new CostContext(bestGroupExpression, entry.getValue().getCost()),
