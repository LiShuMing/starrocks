...
/*
 *
 * Rewrite grouping sets to union all. eg:
 *      select a, b, count(1) from t group by rollup(a, b);
 * rewrite to:
 *      select a, b, count(1) from t group by a, b
 *      union all
 *      select a, null, count(1) from t group by a
 *      union all
 *      select null, b, count(1) from t group by b
 *      union all
 *      select null, null, count(1) from t
 */
    ...
    public RewriteGroupingSetsByCTERule() {
        super(RuleType.TF_REWRITE_GROUPING_SET,
                Pattern.create(OperatorType.LOGICAL_AGGR)
                        .addChildren(Pattern.create(OperatorType.LOGICAL_REPEAT, OperatorType.LOGICAL_PROJECT)));
    }

    @Override
    public boolean check(OptExpression input, OptimizerContext context) {
        LogicalAggregationOperator aggregate = (LogicalAggregationOperator) input.getOp();
        return aggregate.getType() == AggType.GLOBAL;
    }
