...
public class EnforceAndCostTask extends OptimizerTask implements Cloneable {
    ...
    private void setPropertyWithCost(GroupExpression groupExpression,
        ...
        this.groupExpression.getGroup().setBestExpression(new CostContext(groupExpression, curTotalCost), requiredProperty);
    }
    ...
    private void updateCostWithEnforcer(GroupExpression enforcer,
        ...
        groupExpression.getGroup().setBestExpression(new CostContext(enforcer, curTotalCost), newOutputProperty);
    }
    ...
    private PhysicalPropertySet updateCostAndOutputPropertySet(GroupExpression enforcer,
        ...
        PhysicalPropertySet newOutputProperty = groupExpression.getGroup().updateOutputPropertySet(
                new CostContext(enforcer, curTotalCost), requiredPropertySet);
        if (isUpdateSatisfyRequiredProperty(enforcer, newOutputProperty, Lists.newArrayList(oldOutputProperty), curTotalCost)) {
            enforcer.setOutputPropertySatisfyRequiredProperty(newOutputProperty, newOutputProperty);
        }
        return newOutputProperty;
