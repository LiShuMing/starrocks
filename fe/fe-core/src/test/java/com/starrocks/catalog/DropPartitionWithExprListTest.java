...
public class DropPartitionWithExprListTest {
    ...
    class PartitionNameValues {
        @SerializedName("pName")
        String partitionName;
        @SerializedName("pValues")
        List<String> values;
        public PartitionNameValues(String partitionName, List<String> values) {
    ...
    public void testJson() {
        List<List<String>> t = Lists.newArrayList();
        starRocksAssert.withTable(T5, (obj) -> {
            String tableName = (String) obj;
            withTablePartitionsV2(tableName);
            OlapTable olapTable = (OlapTable) starRocksAssert.getTable("test", tableName);
            ...
            List<PartitionNameValues> list = Lists.newArrayList();
            for (Map.Entry<Long, List<List<LiteralExpr>>> e : listPartitionInfo.getMultiLiteralExprValues().entrySet()) {
                for (List<LiteralExpr> values : e.getValue()) {
                    List<String> s = Lists.newArrayList();
                    for (LiteralExpr literalExpr : values) {
                        s.add(literalExpr.getStringValue());
                    }
                    list.add(new PartitionNameValues(olapTable.getPartition(e.getKey()).getName(), s));
                }
            }
