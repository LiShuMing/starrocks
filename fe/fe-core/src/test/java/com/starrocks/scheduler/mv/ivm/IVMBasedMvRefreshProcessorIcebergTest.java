...
public class IVMBasedMvRefreshProcessorIcebergTest extends MVIVMIcebergTestBase {
    ...
    @Test
    public void testIVMWithLeftJoin1() throws Exception {
        doTestWith3RunsNoCheckRewrite("SELECT md5(a.id) as __ROW_ID__, a.id * 2 + 1 as col1, b.data as col2 FROM " +
                        "`iceberg0`.`unpartitioned_db`.`t0` a " +
                        "   left join `iceberg0`.`partitioned_db`.`t1` b on a.id=b.id  where a.id > 10;",
                plan -> {
                    PlanTestBase.assertContains(plan.getExplainString(TExplainLevel.COSTS),
                            "     TABLE: unpartitioned_db.t0\n" +
                                    "     PREDICATES: 14: id > 10\n" +
    ...
    @Test
    public void testIVMWithLeftJoin2() {
        Assertions.assertThrows(DmlException.class, () -> {
            doTest("SELECT md5(a.id) as __ROW_ID__, a.id * 2 + 1, b.data, c.a as ca FROM " +
                    "`iceberg0`.`unpartitioned_db`.`t0` a " +
                    "   left join `iceberg0`.`partitioned_db`.`t1` b on a.id=b.id" +
                    "   left join `iceberg0`.`partitioned_db`.`part_tbl1` c on a.id=c.c where a.id > 10");
        });
    }

    @Test
    public void testIVMWithFullJoin1() throws Exception {
        doTestWith3RunsNoCheckRewrite("SELECT md5(a.id) as __ROW_ID__, a.id * 2 + 1 as col1, b.data as col2 FROM " +
                        "`iceberg0`.`unpartitioned_db`.`t0` a " +
                        "   full join `iceberg0`.`partitioned_db`.`t1` b on a.id=b.id  where a.id > 10;",
                plan -> {
                    PlanTestBase.assertContains(plan.getExplainString(TExplainLevel.COSTS),
                            "     TABLE: unpartitioned_db.t0\n" +
                                    "     PREDICATES: 14: id > 10\n" +
    ...
    @Test
    public void testIVMWithFullJoin2() {
        Assertions.assertThrows(DmlException.class, () -> {
            doTest("SELECT md5(a.id) as __ROW_ID__, a.id * 2 + 1, b.data, c.a as ca FROM " +
                    "`iceberg0`.`unpartitioned_db`.`t0` a " +
                    "   full join `iceberg0`.`partitioned_db`.`t1` b on a.id=b.id" +
                    "   full join `iceberg0`.`partitioned_db`.`part_tbl1` c on a.id=c.c where a.id > 10");
        });
    }

    @Test
    public void testIVMWithSemiJoin1() throws Exception {
        doTestWith3Runs("SELECT a.id * 2 + 1 FROM `iceberg0`.`unpartitioned_db`.`t0` a " +
                        "where exists (select b.id from `iceberg0`.`partitioned_db`.`t1` b where a.id=b.id and b.id > 10);",
                plan -> {
                    PlanTestBase.assertContains(plan.getExplainString(TExplainLevel.COSTS),
                            "     TABLE: unpartitioned_db.t0\n" +
                                    "     PREDICATES: 14: id > 10\n" +
    ...
    @Test
    public void testIVMWithSemiJoin2() throws Exception {
        doTestWith3Runs("SELECT a.id * 2 + 1, b.data, c.a as ca FROM " +
                        "`iceberg0`.`unpartitioned_db`.`t0` a " +
                        "   join `iceberg0`.`partitioned_db`.`t1` b join `iceberg0`.`partitioned_db`.`part_tbl1` c " +
                        "   on a.id=b.id and a.id=c.c where a.id > 10;",
