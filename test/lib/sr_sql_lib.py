...
class StarrocksSQLApiLib(object):
    ...
    def _check_mv_to_refresh_partition_membership(self, mv_name, expects, should_contain) -> bool:
        ...
        if expect_no_refresh:
            ...
            return True
        if should_contain:
            tools.assert_true(mv_to_refresh_partitions is not None and len(mv_to_refresh_partitions) > 0,
                              "assert mv %s mvToRefreshPartitions is empty, but expect to refresh: %s" % (mv_name, json_result))
            ...
            tools.assert_true(mv_to_refresh_partitions is None or len(mv_to_refresh_partitions) == 0,
                              "assert mv %s mvToRefreshPartitions is not empty, but expect to no refresh: %s" % (mv_name, json_result))
