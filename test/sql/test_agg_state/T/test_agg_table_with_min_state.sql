...
CREATE TABLE test_agg_tbl1(
  k1 VARCHAR(10),
  k2 agg_state<min(datetime)> agg_state_union,
  ...
  k13 agg_state<min(decimal)> agg_state_union
)
AGGREGATE KEY(k1)
PARTITION BY (k1) 
...
insert into test_agg_tbl1 select k1, min_state(k2), min_state(k6), min_state(k7), min_state(k8), min_state(k9), min_state(k10), min_state(k11), min_state(k12), min_state(k13) from t1;

-- query    
select min_merge(k2), min_merge(k6), min_merge(k7), min_merge(k8), min_merge(k9), min_merge(k10), min_merge(k11), min_merge(k12), min_merge(k13) from test_agg_tbl1;
select k1, min_merge(k2), min_merge(k6), min_merge(k7), min_merge(k8), min_merge(k9), min_merge(k10), min_merge(k11), min_merge(k12), min_merge(k13) from test_agg_tbl1 group by dt order by 1 limit 3;
